...
/**
 * [FinalizacaoAcaoAdapter] é um adaptador para `RecyclerView` que exibe uma lista de [Acao]s
 * que podem ser finalizadas.
 *
 * Cada item da lista apresenta os detalhes de uma ação, como título, descrição, datas,
 * responsável e criador. Ele também inclui um botão para finalizar a ação, que aciona
 ...
 */
class FinalizacaoAcaoAdapter(
    private var acoes: List<Acao>,
    private val onFinalizarAcaoClick: (Acao) -> Unit,
    private val usuarioRepository: UsuarioRepository
) : RecyclerView.Adapter<FinalizacaoAcaoAdapter.AcaoViewHolder>() {

    /**
     * [AcaoViewHolder] é uma classe interna `ViewHolder` que mantém as referências
     * dos componentes de UI de cada item da lista.
     *
     * @param view A `View` raiz do layout do item (`item_finalizacao_acao.xml`).
     */
    ...
    override fun onBindViewHolder(holder: AcaoViewHolder, position: Int) {
        ...
        holder.btnFinalizarAcao.setOnClickListener {
            onFinalizarAcaoClick(acao)
        }
    }
